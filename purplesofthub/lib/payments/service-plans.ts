export type BillingType = 'one-time' | 'monthly' | 'yearly' | 'custom'

export type ServiceCategory =
  | 'development'
  | 'marketing'
  | 'design'
  | 'music'
  | 'support'
  | 'content'

export interface ServicePlan {
  id: string
  name: string
  priceNGN: number
  priceUSD: number
  billingType: BillingType
  delivery: string
  description: string
  features: string[]
  highlighted?: boolean
  isCustom?: boolean
  badge?: string
}

export interface Service {
  id: string
  slug: string
  name: string
  tagline: string
  description: string
  category: ServiceCategory
  icon: string
  startingPriceNGN: number
  startingPriceUSD: number
  billingType: BillingType
  payOnline: boolean
  plans: ServicePlan[]
}

export const SERVICES: Service[] = [
  // ── WEB DEVELOPMENT ──
  {
    id: 'web-development',
    slug: 'web-development',
    name: 'Web Development',
    tagline: 'Professional websites that convert visitors into clients',
    description: 'We build fast, modern, and conversion-optimized websites for businesses, startups, and creators across Africa and worldwide. From simple landing pages to complex SaaS platforms.',
    category: 'development',
    icon: '🌐',
    startingPriceNGN: 450000,
    startingPriceUSD: 300,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'web-starter',
        name: 'Starter',
        priceNGN: 450000,
        priceUSD: 300,
        billingType: 'one-time',
        delivery: '2 weeks',
        description: 'Perfect for small businesses and personal brands',
        features: [
          '1–3 pages',
          'Mobile responsive design',
          'Contact form',
          'Basic SEO setup',
          'WhatsApp chat button',
          '1 month free support',
        ],
      },
      {
        id: 'web-essential',
        name: 'Essential',
        priceNGN: 750000,
        priceUSD: 500,
        billingType: 'one-time',
        delivery: '3 weeks',
        description: 'Ideal for growing businesses needing more pages',
        features: [
          'Up to 5 pages',
          'Mobile responsive design',
          'Contact form + Newsletter',
          'On-page SEO',
          'Google Analytics setup',
          'Social media integration',
          '2 months free support',
        ],
      },
      {
        id: 'web-professional',
        name: 'Professional',
        priceNGN: 975000,
        priceUSD: 650,
        billingType: 'one-time',
        delivery: '4 weeks',
        description: 'For established businesses wanting premium quality',
        features: [
          'Up to 8 pages',
          'Premium UI/UX design',
          'Contact form + Newsletter',
          'Blog/CMS setup',
          'Full SEO optimization',
          'Speed optimization',
          'Payment integration',
          '2 months free support',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'web-advanced',
        name: 'Advanced',
        priceNGN: 1200000,
        priceUSD: 800,
        billingType: 'one-time',
        delivery: '5 weeks',
        description: 'For businesses needing advanced features',
        features: [
          'Up to 12 pages',
          'Custom animations',
          'E-commerce ready',
          'Member/login area',
          'Advanced SEO',
          'Email marketing setup',
          'API integrations',
          '3 months free support',
        ],
      },
      {
        id: 'web-premium',
        name: 'Premium',
        priceNGN: 1800000,
        priceUSD: 1200,
        billingType: 'one-time',
        delivery: '6 weeks',
        description: 'Full-featured website with client portal',
        features: [
          'Unlimited pages',
          'Client dashboard/portal',
          'Full e-commerce',
          'Payment processing',
          'Custom admin panel',
          'Advanced animations',
          'Multi-language support',
          '4 months free support',
        ],
      },
      {
        id: 'web-elite',
        name: 'Elite',
        priceNGN: 2550000,
        priceUSD: 1700,
        billingType: 'one-time',
        delivery: '8 weeks',
        description: 'Enterprise-grade website with SaaS features',
        features: [
          'Everything in Premium',
          'SaaS functionality',
          'Subscription billing',
          'User management system',
          'Advanced analytics',
          'Third-party API integrations',
          '6 months free support',
        ],
      },
      {
        id: 'web-executive',
        name: 'Executive',
        priceNGN: 3270000,
        priceUSD: 2180,
        billingType: 'one-time',
        delivery: '10 weeks',
        description: 'Complete digital platform with mobile app',
        features: [
          'Everything in Elite',
          'Mobile app (iOS + Android)',
          'Custom CRM features',
          'Workflow automation',
          'White-label ready',
          'Dedicated project manager',
          '6 months free support',
        ],
      },
      {
        id: 'web-platinum',
        name: 'Platinum',
        priceNGN: 4845000,
        priceUSD: 2850,
        billingType: 'one-time',
        delivery: '12 weeks',
        description: 'Full digital ecosystem for large businesses',
        features: [
          'Everything in Executive',
          'Full SaaS platform',
          'AI feature integration',
          'Multi-tenant architecture',
          'Advanced security',
          'Performance SLA',
          '12 months free support',
        ],
      },
      {
        id: 'web-diamond',
        name: 'Diamond',
        priceNGN: 8500000,
        priceUSD: 5000,
        billingType: 'one-time',
        delivery: '16+ weeks',
        description: 'Enterprise digital transformation project',
        features: [
          'Everything in Platinum',
          'Dedicated development team',
          'Custom AI/ML features',
          'Enterprise integrations',
          'Staff training',
          'Ongoing development retainer',
          '24 months free support',
        ],
        badge: 'Best Value',
      },
    ],
  },

  // ── FACEBOOK/INSTAGRAM ADS ──
  {
    id: 'facebook-ads',
    slug: 'facebook-and-instagram-ads',
    name: 'Facebook & Instagram Ads',
    tagline: 'Meta advertising that drives real results for your business',
    description: 'Professional Facebook and Instagram ad management. We build, test, and scale campaigns that consistently deliver results. Ad spend is paid directly to Meta.',
    category: 'marketing',
    icon: '📘',
    startingPriceNGN: 42000,
    startingPriceUSD: 30,
    billingType: 'monthly',
    payOnline: true,
    plans: [
      {
        id: 'ads-flex',
        name: 'Flex Weekly',
        priceNGN: 42000,
        priceUSD: 30,
        billingType: 'monthly',
        delivery: 'Weekly',
        description: 'Test campaigns with flexible weekly commitment',
        features: [
          '1 active campaign',
          '2-3 ad creatives/week',
          'Facebook OR Instagram',
          'Basic audience targeting',
          'Weekly performance report',
          'WhatsApp support',
        ],
        badge: 'Best for Testing',
      },
      {
        id: 'ads-starter',
        name: 'Starter',
        priceNGN: 150000,
        priceUSD: 107,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Perfect for businesses starting with paid ads',
        features: [
          '1 active campaign',
          '3-5 ad creatives/month',
          'Facebook OR Instagram',
          'Basic audience targeting',
          'Weekly optimization',
          'Monthly performance report',
          'Recommended ad spend: ₦150K-₦300K',
        ],
      },
      {
        id: 'ads-growth',
        name: 'Growth',
        priceNGN: 250000,
        priceUSD: 178,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Scale your reach with Facebook AND Instagram',
        features: [
          '3 active campaigns',
          '8-10 ad creatives/month',
          'Facebook AND Instagram',
          'Advanced targeting + lookalikes',
          'Retargeting campaigns',
          'A/B testing',
          'Bi-weekly strategy calls',
          'Bi-weekly performance report',
          'Recommended ad spend: ₦300K-₦750K',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'ads-scale',
        name: 'Scale',
        priceNGN: 400000,
        priceUSD: 285,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'For businesses ready to dominate their market',
        features: [
          'Unlimited campaigns',
          'Unlimited ad creatives',
          'Full-funnel strategy',
          'Advanced pixel setup',
          'Custom audiences',
          'Competitor analysis',
          'Weekly strategy calls',
          'Weekly detailed report',
          'Recommended ad spend: ₦750K+',
        ],
      },
      {
        id: 'ads-enterprise',
        name: 'Enterprise',
        priceNGN: 600000,
        priceUSD: 428,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Full-service Meta advertising with dedicated team',
        features: [
          'Everything in Scale',
          'Dedicated account manager',
          'Custom automation & funnels',
          'Creative production support',
          'Multi-account management',
          'Monthly in-depth audit',
          'Priority support 7 days/week',
          'Recommended ad spend: ₦1.5M+',
        ],
      },
    ],
  },

  // ── MOBILE APP DEVELOPMENT ──
  {
    id: 'mobile-app-development',
    slug: 'mobile-app-development',
    name: 'Mobile App Development',
    tagline: 'Cross-platform mobile apps users love on iOS and Android',
    description: 'We build beautiful, high-performance mobile apps using React Native and Flutter. From simple utility apps to complex marketplace platforms — delivered on time.',
    category: 'development',
    icon: '📱',
    startingPriceNGN: 750000,
    startingPriceUSD: 500,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'app-starter',
        name: 'Starter',
        priceNGN: 750000,
        priceUSD: 500,
        billingType: 'one-time',
        delivery: '4-6 weeks',
        description: 'Best for simple MVP apps and startup ideas.',
        features: [
          'Android app build',
          'Up to 5 screens',
          'Basic UI/UX design',
          'User authentication',
          'Simple backend integration',
          'Basic admin support',
          '4-6 weeks delivery',
        ],
      },
      {
        id: 'app-growth',
        name: 'Growth',
        priceNGN: 1500000,
        priceUSD: 1071,
        billingType: 'one-time',
        delivery: '6-10 weeks',
        description: 'Ideal for businesses that need a complete functional mobile app.',
        features: [
          'Android + iOS app build',
          'Up to 12 screens',
          'Premium UI/UX design',
          'User authentication',
          'API/backend integration',
          'Payment integration',
          'Push notifications',
          'Admin dashboard support',
          '6-10 weeks delivery',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'app-pro',
        name: 'Pro',
        priceNGN: 2500000,
        priceUSD: 1785,
        billingType: 'one-time',
        delivery: '10-14 weeks',
        description: 'For advanced apps with custom features and scalable architecture.',
        features: [
          'Android + iOS app build',
          '20+ screens',
          'Advanced UI/UX design',
          'Custom backend/API integration',
          'Payment gateway integration',
          'Push notifications',
          'Admin dashboard',
          'Analytics integration',
          'Deployment guidance',
          '10-14 weeks delivery',
        ],
      },
      {
        id: 'app-enterprise',
        name: 'Enterprise',
        priceNGN: 0,
        priceUSD: 0,
        billingType: 'custom',
        delivery: 'Custom timeline',
        description: 'For complex platforms, marketplaces, SaaS apps, fintech tools, logistics apps, and high-scale products that require custom architecture and dedicated planning.',
        features: [
          'Custom Android + iOS development',
          'Unlimited screens based on scope',
          'Advanced backend architecture',
          'Custom admin dashboard',
          'Payment gateway integration',
          'Notifications and analytics',
          'Third-party integrations',
          'Security planning',
          'Deployment support',
          'Ongoing maintenance available',
        ],
        isCustom: true,
        badge: 'Contact Us',
      },
    ],
  },

  // ── SOCIAL MEDIA MANAGEMENT ──
  {
    id: 'social-media-management',
    slug: 'social-media-management',
    name: 'Social Media Management',
    tagline: 'Consistent, engaging social media presence that grows your brand',
    description: 'We manage your social media accounts with strategic content, eye-catching graphics, and community engagement. Focus on your business while we grow your audience.',
    category: 'marketing',
    icon: '📣',
    startingPriceNGN: 75000,
    startingPriceUSD: 53,
    billingType: 'monthly',
    payOnline: true,
    plans: [
      {
        id: 'smm-starter',
        name: 'Starter',
        priceNGN: 75000,
        priceUSD: 53,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Perfect for small businesses that need consistent social media presence.',
        features: [
          '8 posts per month',
          'Basic content calendar',
          'Caption writing',
          'Hashtag research',
          'Page optimization',
          'Monthly performance summary',
        ],
      },
      {
        id: 'smm-growth',
        name: 'Growth',
        priceNGN: 150000,
        priceUSD: 107,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Best for brands that want stronger visibility and better audience engagement.',
        features: [
          '15 posts per month',
          'Content calendar',
          'Caption writing',
          'Hashtag research',
          'Community engagement',
          'Basic graphic design support',
          'Monthly analytics report',
          'Growth recommendations',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'smm-scale',
        name: 'Scale',
        priceNGN: 250000,
        priceUSD: 178,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'For growing brands that need consistent content, engagement, and strategy.',
        features: [
          '25 posts per month',
          'Full content strategy',
          'Caption writing',
          'Hashtag research',
          'Community management',
          'Graphic design support',
          'Monthly analytics report',
          'Competitor review',
          'Growth strategy session',
        ],
      },
      {
        id: 'smm-enterprise',
        name: 'Enterprise',
        priceNGN: 500000,
        priceUSD: 357,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'For companies, public figures, and growing brands that need full social media operations and dedicated strategy.',
        features: [
          '40+ posts per month',
          'Advanced content strategy',
          'Dedicated social media manager',
          'Community management',
          'Monthly campaign planning',
          'Advanced analytics report',
          'Competitor tracking',
          'Strategy consultation',
          'Priority support',
        ],
      },
    ],
  },

  // ── MUSIC PROMOTION ──
  {
    id: 'music-promotion',
    slug: 'music-promotion',
    name: 'Music Promotion',
    tagline: 'Get your music heard by millions across Africa and beyond',
    description: 'Strategic music promotion campaigns across TikTok, Instagram, YouTube, and streaming platforms. We help artists build real fanbase and streaming numbers.',
    category: 'music',
    icon: '🎵',
    startingPriceNGN: 75000,
    startingPriceUSD: 50,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'music-spotify-starter',
        name: 'Spotify Starter',
        priceNGN: 75000,
        priceUSD: 50,
        billingType: 'one-time',
        delivery: '7-10 days',
        description: 'Spotify-focused starter push for one release',
        features: [
          '1 song campaign',
          'Spotify playlist pitching',
          'Audience and genre targeting',
          'Save/share growth focus',
          'Starter campaign report',
        ],
      },
      {
        id: 'music-influencer-push',
        name: 'Influencer Push',
        priceNGN: 100000,
        priceUSD: 70,
        billingType: 'one-time',
        delivery: '10-14 days',
        description: 'Short-form creator and influencer campaign for one song',
        features: [
          'TikTok/Reels creator push',
          'Influencer story/post mentions',
          'Content angle direction',
          'Campaign tracking',
          'Performance summary',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'music-youtube-growth',
        name: 'YouTube Growth',
        priceNGN: 120000,
        priceUSD: 80,
        billingType: 'one-time',
        delivery: '14-21 days',
        description: 'Music video, Shorts, visualizer, or YouTube discovery campaign',
        features: [
          'YouTube video promotion',
          'Shorts campaign planning',
          'Audience targeting',
          'Thumbnail/title guidance',
          'Detailed campaign report',
        ],
      },
      {
        id: 'music-meta-ads',
        name: 'Meta Ads Setup',
        priceNGN: 75000,
        priceUSD: 50,
        billingType: 'one-time',
        delivery: '5-7 days',
        description: 'Instagram and Facebook ads setup for music campaigns. Ad budget is separate.',
        features: [
          'Campaign setup',
          'Audience targeting',
          'Creative direction',
          'Pixel/link tracking guidance',
          'Ad budget managed separately',
        ],
      },
      {
        id: 'music-full-rollout',
        name: 'Full Rollout',
        priceNGN: 300000,
        priceUSD: 200,
        billingType: 'one-time',
        delivery: '30 days',
        description: 'Combined rollout across Spotify, creators, YouTube, and social ads',
        features: [
          'Release rollout strategy',
          'Spotify campaign',
          'Influencer/content push',
          'YouTube promotion',
          'Meta Ads planning',
          'Weekly updates',
          'Full analytics report',
        ],
      },
      {
        id: 'music-label',
        name: 'Label Deal',
        priceNGN: 0,
        priceUSD: 0,
        billingType: 'custom',
        delivery: 'Custom',
        description: 'Full artist campaign management like a record label',
        features: [
          'Full album campaign',
          'Distribution included',
          'Brand partnerships',
          'Radio pitching',
          'TV/media appearances',
          'Merchandise strategy',
          'Dedicated campaign manager',
          'Monthly strategy sessions',
        ],
        isCustom: true,
        badge: 'Contact Us',
      },
    ],
  },

  // ── MUSIC DISTRIBUTION ──
  {
    id: 'music-distribution',
    slug: 'music-distribution',
    name: 'Music Distribution',
    tagline: 'Get your music on 150+ platforms including Spotify and Apple Music',
    description: 'Professional music distribution to all major streaming platforms worldwide. Keep most of your royalties and get paid monthly. ISRC and UPC codes included.',
    category: 'music',
    icon: '🎧',
    startingPriceNGN: 15000,
    startingPriceUSD: 10,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'dist-single',
        name: 'Single',
        priceNGN: 15000,
        priceUSD: 10,
        billingType: 'one-time',
        delivery: '2-5 business days',
        description: 'Distribute one single to all major platforms',
        features: [
          '1 single (up to 2 tracks)',
          '150+ streaming platforms',
          'Spotify, Apple Music, Boomplay',
          'ISRC code included',
          'Keep 80% royalties',
          'Monthly royalty payments',
          'Streaming stats dashboard',
        ],
      },
      {
        id: 'dist-album',
        name: 'Album / EP',
        priceNGN: 35000,
        priceUSD: 25,
        billingType: 'one-time',
        delivery: '2-5 business days',
        description: 'Distribute your EP or full album worldwide',
        features: [
          'Up to 15 tracks',
          '150+ streaming platforms',
          'All major platforms worldwide',
          'UPC + ISRC codes included',
          'Keep 85% royalties',
          'Monthly royalty payments',
          'Streaming analytics',
          'Playlist pitching support',
        ],
        highlighted: true,
        badge: 'Best Value',
      },
      {
        id: 'dist-artist',
        name: 'Artist Plan',
        priceNGN: 75000,
        priceUSD: 53,
        billingType: 'yearly',
        delivery: '2-5 business days',
        description: 'Unlimited releases for one full year',
        features: [
          'Unlimited releases per year',
          '150+ streaming platforms',
          'All platforms worldwide',
          'UPC + ISRC codes included',
          'Keep 90% royalties',
          'Monthly royalty payments',
          'Priority distribution',
          'Dedicated artist support',
          'Free music promotion boost',
        ],
      },
      {
        id: 'dist-label',
        name: 'Label Plan',
        priceNGN: 180000,
        priceUSD: 120,
        billingType: 'yearly',
        delivery: '2-5 business days',
        description: 'Distribution support for managers, small labels, and artist collectives',
        features: [
          'Multiple artist support',
          'Unlimited coordinated releases',
          '150+ streaming platforms',
          'UPC + ISRC codes included',
          'Release calendar planning',
          'Priority metadata review',
          'Dedicated label support',
        ],
      },
    ],
  },

  // ── ACCOUNT RECOVERY ──
  {
    id: 'account-recovery',
    slug: 'account-recovery',
    name: 'Account Recovery',
    tagline: 'Professional recovery for hacked or disabled social media accounts',
    description: 'Specialist recovery service for suspended, hacked, or disabled social media accounts. We use official appeal processes with high success rates. Full refund if unsuccessful.',
    category: 'support',
    icon: '🔐',
    startingPriceNGN: 42000,
    startingPriceUSD: 30,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'recovery-facebook',
        name: 'Facebook Recovery',
        priceNGN: 42000,
        priceUSD: 30,
        billingType: 'one-time',
        delivery: '14-30 business days',
        description: 'Recover hacked or disabled Facebook account or page',
        features: [
          'Personal or business account',
          'Facebook page recovery',
          'Official appeal process',
          'ID verification support',
          '14-30 business days',
          '70% success rate',
          'Progress updates via WhatsApp',
          'Full refund if unsuccessful',
        ],
      },
      {
        id: 'recovery-instagram',
        name: 'Instagram Recovery',
        priceNGN: 75000,
        priceUSD: 53,
        billingType: 'one-time',
        delivery: '14-30 business days',
        description: 'Recover hacked or disabled Instagram account',
        features: [
          'Personal or creator account',
          'Business account recovery',
          'Official Meta appeal process',
          'ID verification support',
          '14-30 business days',
          '70% success rate',
          'Progress updates via WhatsApp',
          'Full refund if unsuccessful',
        ],
      },
      {
        id: 'recovery-tiktok',
        name: 'TikTok Recovery',
        priceNGN: 75000,
        priceUSD: 53,
        billingType: 'one-time',
        delivery: '14-30 business days',
        description: 'Recover banned or disabled TikTok account',
        features: [
          'Personal or creator account',
          'TikTok official appeal',
          'ID verification support',
          '14-30 business days',
          '70% success rate',
          'Progress updates via WhatsApp',
          'Full refund if unsuccessful',
        ],
      },
      {
        id: 'recovery-twitter',
        name: 'Twitter/X Recovery',
        priceNGN: 60000,
        priceUSD: 42,
        billingType: 'one-time',
        delivery: '14-21 business days',
        description: 'Recover suspended Twitter/X account',
        features: [
          'Personal or business account',
          'X official appeal process',
          'ID verification support',
          '14-21 business days',
          '65% success rate',
          'Progress updates via WhatsApp',
          'Full refund if unsuccessful',
        ],
      },
      {
        id: 'recovery-gmail',
        name: 'Gmail/Google Recovery',
        priceNGN: 50000,
        priceUSD: 35,
        billingType: 'one-time',
        delivery: '7-21 business days',
        description: 'Recover locked Google or Gmail account',
        features: [
          'Gmail or Google account',
          'Google official recovery',
          'ID verification support',
          '7-21 business days',
          '75% success rate',
          'Progress updates via WhatsApp',
          'Full refund if unsuccessful',
        ],
      },
    ],
  },

  // ── UI/UX DESIGN ──
  {
    id: 'ui-ux-design',
    slug: 'ui-ux-design',
    name: 'UI/UX Design',
    tagline: 'Pixel-perfect designs that users love and convert',
    description: 'We design beautiful, intuitive interfaces for web apps, mobile apps, and dashboards. From wireframes to interactive prototypes — with full Figma handoff.',
    category: 'design',
    icon: '🎨',
    startingPriceNGN: 150000,
    startingPriceUSD: 107,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'ux-basic',
        name: 'Basic',
        priceNGN: 150000,
        priceUSD: 107,
        billingType: 'one-time',
        delivery: '5-7 business days',
        description: 'Best for simple landing pages, small websites, and early-stage ideas.',
        features: [
          'Up to 3 screens',
          'Basic wireframe',
          'Clean UI layout',
          'Mobile responsive design',
          'Figma file delivery',
          '2 revision rounds',
          '5-7 business days',
        ],
      },
      {
        id: 'ux-standard',
        name: 'Standard',
        priceNGN: 300000,
        priceUSD: 214,
        billingType: 'one-time',
        delivery: '7-14 business days',
        description: 'Perfect for websites, dashboards, and growing digital products.',
        features: [
          'Up to 8 screens',
          'Wireframes included',
          'High-fidelity UI design',
          'Mobile + desktop layouts',
          'Interactive prototype',
          'Basic design system',
          'Developer handoff',
          '3 revision rounds',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'ux-premium',
        name: 'Premium',
        priceNGN: 600000,
        priceUSD: 428,
        billingType: 'one-time',
        delivery: '14-21 business days',
        description: 'For advanced apps, SaaS platforms, and full product design systems.',
        features: [
          'Up to 15 screens',
          'User flow planning',
          'Wireframes included',
          'Premium UI design',
          'Mobile + desktop layouts',
          'Interactive prototype',
          'Full design system elements',
          'Developer handoff',
          '5 revision rounds',
        ],
      },
      {
        id: 'ux-enterprise',
        name: 'Enterprise',
        priceNGN: 0,
        priceUSD: 0,
        billingType: 'custom',
        delivery: 'Custom timeline',
        description: 'For large platforms, complex SaaS products, enterprise dashboards, and full product design systems that require deep UX planning and advanced design execution.',
        features: [
          'Custom number of screens based on scope',
          'UX research and user journey mapping',
          'Full product design system',
          'Advanced dashboard design',
          'Mobile and desktop design',
          'Interactive prototype',
          'Developer handoff documentation',
          'Design review sessions',
          'Priority support',
        ],
        isCustom: true,
        badge: 'Contact Us',
      },
    ],
  },

  // ── LOGO DESIGN ──
  {
    id: 'logo-design',
    slug: 'logo-design',
    name: 'Logo Design',
    tagline: 'Memorable logos that represent your brand perfectly',
    description: 'Professional logo design that captures your brand identity. From simple clean logos to full brand identity packages — all delivered in print and digital ready formats.',
    category: 'design',
    icon: '✏️',
    startingPriceNGN: 25000,
    startingPriceUSD: 17,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'logo-basic',
        name: 'Basic',
        priceNGN: 25000,
        priceUSD: 17,
        billingType: 'one-time',
        delivery: '3-5 business days',
        description: 'Clean professional logo for startups and small businesses',
        features: [
          '2 unique logo concepts',
          'PNG + JPG file formats',
          'White + dark background versions',
          'Transparent background PNG',
          'Social media ready sizes',
          '2 revision rounds',
          '3-5 business days',
        ],
      },
      {
        id: 'logo-standard',
        name: 'Standard',
        priceNGN: 60000,
        priceUSD: 42,
        billingType: 'one-time',
        delivery: '5-7 business days',
        description: 'Professional logo with full file package',
        features: [
          '4 unique logo concepts',
          'All file formats (AI, PDF, PNG, SVG)',
          'White + dark + color versions',
          'Favicon version',
          'Simple brand color guide',
          'Font recommendations',
          'Unlimited revisions',
          '5-7 business days',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'logo-premium',
        name: 'Premium',
        priceNGN: 120000,
        priceUSD: 85,
        billingType: 'one-time',
        delivery: '7-10 business days',
        description: 'Complete logo with brand style guide',
        features: [
          '6 unique logo concepts',
          'All file formats',
          'Complete brand style guide',
          'Color palette (primary + secondary)',
          'Typography guide',
          'Business card mockup',
          'Social media profile kit',
          'Letterhead mockup',
          'Unlimited revisions',
        ],
      },
      {
        id: 'logo-brand',
        name: 'Brand Identity',
        priceNGN: 250000,
        priceUSD: 178,
        billingType: 'one-time',
        delivery: '2-3 weeks',
        description: 'Full brand identity package for serious businesses',
        features: [
          'Everything in Premium',
          'Full brand identity system',
          'Business card design (print-ready)',
          'Letterhead + invoice template',
          'Email signature design',
          'Social media templates (10)',
          'Brand guidelines document (PDF)',
          'Brand launch strategy',
          'Unlimited revisions',
        ],
      },
    ],
  },

  // ── SEO & CONTENT ──
  {
    id: 'seo-content',
    slug: 'seo-content',
    name: 'SEO & Content Marketing',
    tagline: 'Rank higher on Google and drive organic traffic that converts',
    description: 'Strategic SEO and content marketing to grow your organic traffic. We handle keyword research, on-page optimization, backlink building, and content creation.',
    category: 'marketing',
    icon: '🔍',
    startingPriceNGN: 40000,
    startingPriceUSD: 28,
    billingType: 'monthly',
    payOnline: true,
    plans: [
      {
        id: 'seo-basic',
        name: 'Basic',
        priceNGN: 40000,
        priceUSD: 28,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Start your SEO journey with essential optimizations',
        features: [
          '5 blog posts per month',
          'Keyword research (20 keywords)',
          'On-page SEO optimization',
          'Google Search Console setup',
          'Google Analytics setup',
          'Meta tags optimization',
          'Monthly ranking report',
        ],
      },
      {
        id: 'seo-standard',
        name: 'Standard',
        priceNGN: 80000,
        priceUSD: 57,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Comprehensive SEO for growing businesses',
        features: [
          '10 blog posts per month',
          'Keyword research (50 keywords)',
          'Full technical SEO audit',
          'Backlink building (10/month)',
          'Local SEO optimization',
          'Competitor analysis',
          'Schema markup',
          'Weekly ranking report',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'seo-premium',
        name: 'Premium',
        priceNGN: 150000,
        priceUSD: 107,
        billingType: 'monthly',
        delivery: 'Monthly',
        description: 'Full SEO management for market dominance',
        features: [
          '20 blog posts per month',
          'Full keyword strategy',
          'Technical SEO management',
          'Backlink building (30/month)',
          'Press release distribution',
          'Video SEO optimization',
          'Content strategy planning',
          'Dedicated SEO manager',
          'Daily rank monitoring',
          'Monthly strategy meeting',
        ],
      },
    ],
  },

  // ── VIDEO CONTENT ──
  {
    id: 'video-content',
    slug: 'video-content-creation',
    name: 'Video Content Creation',
    tagline: 'Professional videos that capture attention and drive engagement',
    description: 'High-quality video production for social media, brand campaigns, product demos, and more. From concept to final delivery — optimized for all platforms.',
    category: 'content',
    icon: '🎬',
    startingPriceNGN: 50000,
    startingPriceUSD: 35,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'video-basic',
        name: 'Basic',
        priceNGN: 50000,
        priceUSD: 35,
        billingType: 'one-time',
        delivery: '3-5 business days',
        description: 'Short social media video for one platform',
        features: [
          '1 short video (up to 60 sec)',
          'Social media optimized',
          'Basic editing + transitions',
          'Background music',
          'Captions/subtitles',
          '1 platform format',
          '1 revision round',
        ],
      },
      {
        id: 'video-standard',
        name: 'Standard',
        priceNGN: 120000,
        priceUSD: 85,
        billingType: 'one-time',
        delivery: '5-7 business days',
        description: 'Multiple short videos or one longer video',
        features: [
          '3 short videos OR 1 long video (5-10 min)',
          'Professional editing',
          'Motion graphics + animations',
          'Brand colors/logo integration',
          'Multiple format delivery',
          'Background music',
          'Captions/subtitles',
          '2 revision rounds',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'video-premium',
        name: 'Premium',
        priceNGN: 250000,
        priceUSD: 178,
        billingType: 'one-time',
        delivery: '7-14 business days',
        description: 'Premium video package with scriptwriting',
        features: [
          '5 short videos OR 2 long videos',
          'Script writing included',
          'Storyboard planning',
          'Professional voiceover',
          'Advanced motion graphics',
          'Brand animations',
          'All platform formats',
          'Unlimited revisions',
        ],
      },
      {
        id: 'video-brand',
        name: 'Brand Video',
        priceNGN: 500000,
        priceUSD: 357,
        billingType: 'one-time',
        delivery: '2-4 weeks',
        description: 'Full brand or promotional video production',
        features: [
          'Full brand/promo video (2-5 min)',
          'Script + storyboard',
          'Professional voiceover',
          '3D animations if needed',
          'Color grading',
          'Sound design',
          'All platform formats',
          'Unlimited revisions',
          'Usage rights included',
        ],
      },
    ],
  },

  // ── BRANDING ──
  {
    id: 'branding',
    slug: 'branding-creative-design',
    name: 'Branding & Creative Design',
    tagline: 'Build a brand identity that commands attention and trust',
    description: 'Complete brand identity creation from strategy to execution. We craft visual identities that tell your story, differentiate you from competitors, and connect with your target audience.',
    category: 'design',
    icon: '💎',
    startingPriceNGN: 150000,
    startingPriceUSD: 107,
    billingType: 'one-time',
    payOnline: true,
    plans: [
      {
        id: 'brand-starter',
        name: 'Starter Brand',
        priceNGN: 150000,
        priceUSD: 107,
        billingType: 'one-time',
        delivery: '1-2 weeks',
        description: 'Essential brand identity for new businesses',
        features: [
          'Logo design (3 concepts)',
          'Brand color palette',
          'Typography selection',
          'Basic brand guidelines (PDF)',
          'Business card design',
          'Email signature design',
          'PNG + SVG files',
          '2 revision rounds',
        ],
      },
      {
        id: 'brand-full',
        name: 'Full Brand',
        priceNGN: 350000,
        priceUSD: 250,
        billingType: 'one-time',
        delivery: '2-4 weeks',
        description: 'Complete brand identity for established businesses',
        features: [
          'Everything in Starter',
          'Full brand strategy document',
          'Social media templates (20)',
          'Letterhead + invoice template',
          'Proposal/presentation template',
          'Brand pattern/texture design',
          'Comprehensive brand guide',
          'Unlimited revisions',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'brand-premium',
        name: 'Premium Brand',
        priceNGN: 600000,
        priceUSD: 428,
        billingType: 'one-time',
        delivery: '4-6 weeks',
        description: 'Premium brand with website and video',
        features: [
          'Everything in Full Brand',
          'Landing page design',
          'Brand video (30 sec)',
          'Packaging design mockup',
          'Marketing materials design',
          'Brand launch strategy',
          'Dedicated brand manager',
          'Unlimited revisions',
        ],
      },
      {
        id: 'brand-enterprise',
        name: 'Enterprise Brand',
        priceNGN: 0,
        priceUSD: 0,
        billingType: 'custom',
        delivery: 'Custom timeline',
        description: 'Complete brand overhaul for large organizations',
        features: [
          'Everything in Premium',
          'Full market research',
          'Brand positioning strategy',
          'Complete rebrand management',
          'Staff brand training',
          'Ongoing brand management',
          'Quarterly brand audits',
        ],
        isCustom: true,
        badge: 'Contact Us',
      },
    ],
  },

  // ── SAAS DEVELOPMENT ──
  {
    id: 'saas-development',
    slug: 'saas-development',
    name: 'SaaS Development',
    tagline: 'Build scalable SaaS products that generate revenue',
    description: 'End-to-end SaaS product development for founders, startups, and businesses. We build scalable software platforms with secure authentication, dashboards, subscriptions, payments, and product workflows that are ready for launch and long-term growth.',
    category: 'development',
    icon: '⚡',
    startingPriceNGN: 1500000,
    startingPriceUSD: 1071,
    billingType: 'one-time',
    payOnline: false,
    plans: [
      {
        id: 'saas-mvp',
        name: 'MVP',
        priceNGN: 1500000,
        priceUSD: 1071,
        billingType: 'one-time',
        delivery: '6-10 weeks',
        description: 'Best for founders who want to validate a SaaS idea quickly with core features.',
        features: [
          'MVP product planning',
          'Up to 8 core screens',
          'User authentication',
          'Basic admin dashboard',
          'Database setup',
          'Payment integration',
          'Basic subscription flow',
          'Deployment support',
          '6-10 weeks delivery',
        ],
      },
      {
        id: 'saas-growth',
        name: 'Growth',
        priceNGN: 3000000,
        priceUSD: 2142,
        billingType: 'one-time',
        delivery: '10-16 weeks',
        description: 'For startups and businesses that need a stronger SaaS platform with user roles, dashboards, and scalable workflows.',
        features: [
          'Full SaaS product planning',
          'Up to 18 core screens',
          'User roles and permissions',
          'Advanced admin dashboard',
          'Client/user dashboard',
          'Subscription billing setup',
          'Payment gateway integration',
          'Email notifications',
          'Analytics-ready structure',
          'Deployment support',
          '10-16 weeks delivery',
        ],
        highlighted: true,
        badge: 'Most Popular',
      },
      {
        id: 'saas-enterprise',
        name: 'Enterprise',
        priceNGN: 0,
        priceUSD: 0,
        billingType: 'custom',
        delivery: 'Based on scope',
        description: 'For advanced SaaS platforms, marketplaces, dashboards, automation tools, fintech products, client portals, and subscription-based systems that require custom architecture and deeper planning.',
        features: [
          'Custom SaaS architecture',
          'Unlimited screens based on project scope',
          'Advanced user roles and permissions',
          'Multi-dashboard system',
          'Advanced subscription logic',
          'API and third-party integrations',
          'Automation workflows',
          'Secure database planning',
          'Performance optimization',
          'Deployment and launch support',
          'Maintenance plan available',
        ],
        isCustom: true,
        badge: 'Custom Quote',
      },
    ],
  },
]

// ── HELPER FUNCTIONS ──

export function getServiceBySlug(slug: string): Service | undefined {
  return SERVICES.find(s => s.slug === slug || s.id === slug)
}

export function getServicesByCategory(category: ServiceCategory): Service[] {
  return SERVICES.filter(s => s.category === category)
}

export function formatPrice(amount: number, currency: 'NGN' | 'USD' = 'NGN'): string {
  if (amount === 0) return 'Custom'
  if (currency === 'NGN') {
    return `₦${amount.toLocaleString()}`
  }
  return `$${amount.toLocaleString()}`
}

export function getPlanById(planId: string): ServicePlan | undefined {
  for (const service of SERVICES) {
    const plan = service.plans.find(p => p.id === planId)
    if (plan) return plan
  }
  return undefined
}

export function getServiceByPlanId(planId: string): Service | undefined {
  return SERVICES.find(s => s.plans.some(p => p.id === planId))
}

export const EXCHANGE_RATE = 1400
// ₦1,400 = $1 USD

