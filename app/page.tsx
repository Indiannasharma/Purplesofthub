...
export default function Home() {
  return (
    <main style={{ background: "var(--bg-primary)", color: "var(--text-primary)", minHeight: "100vh" }}>
          ...
          {[
            ...
            <Reveal key={c.title} delay={i * 0.1}>
              <div className="glass-card" style={{ padding: "28px 28px 24px", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ color: "var(--text-muted)", fontSize: 14, marginBottom: 20 }}>{c.sub}</div>
                <Link href={c.href}>
                  <button style={{ background: "rgba(124,58,237,.12)", border: "1px solid rgba(168,85,247,.3)", borderRadius: 100, padding: "8px 18px", color: "var(--accent)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                    {c.cta} →
                  </button>
                </Link>
              </div>
      ...
      <section id="music" style={{ padding: "100px 5%", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 2, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          <Reveal>
            ...
            <h2 style={{ fontFamily: "Outfit", fontSize: "clamp(28px,3.5vw,46px)", fontWeight: 900, color: "var(--text-primary)", letterSpacing: "-1.5px", lineHeight: 1.15, marginBottom: 20 }}>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: 16, lineHeight: 1.85, marginBottom: 32 }}>We help artists get heard worldwide. From Spotify & Apple Music distribution to social media campaigns that build real, loyal fanbases.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 36 }}>
              {["🎵 Distribute to 150+ platforms globally", "📱 TikTok, Reels & YouTube promo campaigns", "🎨 Artist branding & cover art design", "📊 Stream analytics & audience insights"].map(t => (
                <div key={t} style={{ color: "var(--text-secondary)", fontSize: 15 }}>{t}</div>
              ))}
            </div>
            <Link href="/contact">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {[["🎧","150+ Platforms","Global distribution"],["📈","Real Growth","Authentic streams"],["🎨","Artist Branding","Visual identity"],["📢","Promotion","Targeted campaigns"]].map(([ic, tt, dd]) => (
                <div key={tt} className="glass-card" style={{ padding: "24px 18px", textAlign: "center" }}>
                  ...
                  <div style={{ fontWeight: 800, color: "var(--text-primary)", fontSize: 15, marginBottom: 6 }}>{tt}</div>
                  <div style={{ color: "var(--text-muted)", fontSize: 13 }}>{dd}</div>
                </div>
              ))}
            </div>
          </Reveal>
      ...
      {/* ── SUCCESS STORIES ── */}
      <section style={{ padding: "90px 5%", background: "var(--bg-secondary)", borderTop: "1px solid rgba(124,58,237,.12)", borderBottom: "1px solid rgba(124,58,237,.12)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 60 }}>
                ...
                <div className="glass-card" style={{ padding: "28px 24px", position: "relative", overflow: "hidden" }}>
                  ...
                  {[["Challenge", s.challenge], ["Solution", s.solution], ["Result", s.result]].map(([label, val]) => (
                    <div key={label} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: label === "Result" ? "#a855f7" : "var(--text-muted)", minWidth: 65 }}>{label}:</span>
      ...
      <section style={{ padding: "90px 5%", background: "rgba(124,58,237,.04)", borderTop: "1px solid rgba(124,58,237,.12)", borderBottom: "1px solid rgba(124,58,237,.12)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 60 }}>
              <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, color: "var(--accent)", textTransform: "uppercase", marginBottom: 12 }}>Client Love</p>
      ...
      <section style={{ padding: "100px 5%", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <Reveal>
          <div style={{ position: "relative", zIndex: 2, maxWidth: 680, margin: "0 auto" }}>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: 17, marginBottom: 40, lineHeight: 1.7 }}>Partner with us to elevate your business. Your next big digital move starts here.</p>
